...
describe("stats/general", function(){
    ...
    beforeEach(function(done) {
        ...
        db.model('GameRecord', gameRecordSchema).create([gameRecord1, gameRecord2, gameRecord3], function() {
            done();
        });
    });
    ...
    it("mostGames is calculated correctly", function(done) {
        request(server)
            .get("/api/stats/players?records=2")
            .set('Accept', 'application/json')
            .end(function(err, res) {
                var bodyObj = JSON.parse(res.text);
                expect(bodyObj).to.have.propertyVal('mostGames.player', 'flend');
                done();
            });
    });
